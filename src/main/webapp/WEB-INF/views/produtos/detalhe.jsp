...
<body>
  ...
  <header id="layout-header">
	...
	<nav class="categories-nav">
		<ul class="container">
			<li class="category"><a href="${contextPath}produtos">Home</a></li>
		</ul>
	</nav>
	  ...
	  <!-- <form action="${s:mvcUrl('CCC#add').build() }" method="post" class="container">
	    ...
	    <ul id="variants" class="clearfix">
	    	  <c:forEach items="${produto.precos}" var="preco">
		    	  	<li class="buy-option">
		            <input type="radio" name="id" class="variant-radio" id="product-variant-9720393823" value="${preco.tipo}" checked="checked" />
		            <label  class="variant-label" for="product-variant-9720393823">
		              ${preco.tipo } 
		            </label>
		            <small class="compare-at-price">R$ ${preco.valor } - ID: ${produto.id}</small>
		          </li>
	    	  </c:forEach>
	    	             
	    </ul>
	    <button type="submit" class="submit-image icon-basket-alt" title="Compre Agora">COMPRAR</button>
